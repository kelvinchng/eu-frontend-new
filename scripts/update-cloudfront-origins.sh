...
# CloudFront domains to distribution ID mapping
declare -A DISTRIBUTIONS
DISTRIBUTIONS["d3b1dndgqrts0z.cloudfront.net"]="prod"
DISTRIBUTIONS["di16jbtvptky0.cloudfront.net"]="staging"

echo "🔧 Updating CloudFront distributions to use S3 website endpoints..."

# First, let's find the distribution IDs
...
update_distribution() {
    ...
    jq --arg bucket "$BUCKET.s3-website-$AWS_REGION.amazonaws.com" \
       '.DistributionConfig.Origins.Items[0].DomainName = $bucket | 
        ...
        del(.DistributionConfig.Origins.Items[0].S3OriginConfig)' /tmp/dist-config.json | \
